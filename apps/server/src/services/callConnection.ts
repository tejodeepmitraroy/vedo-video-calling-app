...
type userIdToSocketIdMap = Map<string, string>;
...
export function callConnections(
	socket: Socket,
	io: Server,
	rooms: RoomDetails,
	socketIdToUserIdMap: socketIdToUserIdMap,
	userIdToSocketIdMap: userIdToSocketIdMap,
	hostSocketIdToRoomId: hostSocketIdToRoomId
) {
	socket.on(
		'callUser',
		({ to, userName }: { to: string; userName: string }) => {
			const socketId = userIdToSocketIdMap.get(to);

			if (socketId) {
				io.to(socketId).emit('notification:aUserIsCalling', {
					callerSocketId: socket.id,
					userName,
				});
			} else {
				socket.emit('notification:userIsNotOnline');
			}
		}
	);
	socket.on(
		'event:callRejected',
		({ callerSocketId }: { callerSocketId: string }) => {
			io.to(callerSocketId).emit('notification:callRejected');
		}
	);
	...
	socket.on(
		'event:callAccepted',
		({ callerSocketId }: { callerSocketId: string }) => {
			io.to(callerSocketId).emit('notification:callAccepted');
		}
	);
